...
| `input.lua` | Polls keyboard each frame; A/D or arrows = move, E = pick up/down, F = interact; W/S or up/down = `move_up`/`move_down` (menu nav); F/gamepad-A = `interact` (also confirms menus) |
...
| `settings_state.lua` | Holds `fullscreen` bool, `sfx_volume`/`music_volume` ints (0–100), and `keybinds` table (7 actions including `cancel`); owns `love.window.setFullscreen`; mutated exclusively by `SettingsMenu` |
...
| File | What it does |
|------|-------------|
| `start_scene.lua` | Title screen with New Game / Continue / Settings / Exit buttons; uses `input:pressed("move_up"/"move_down"/"interact")` for navigation; accepts `open_settings` callback (4th constructor arg) called when Settings selected; New Game and Continue both enter StoreScene |
...
- **wip-parity-5** — Thirteen gaps closed: `customer_scripts.lua` copied from wip (Romeo ch1-3, Glen ch1-3, Frogsby ch3, Mechafrog ch3, Mayor Bloom 3-chapter rebranding arc, Dottie rebranded as circus clown, `voice_pitch` on all entries, `no_dismiss = true` on sage ch1, rebalanced triggers); speed tiers expanded from 3 to 6 with secondary shoe color; `Player:set_speed_color(color, secondary)` stores and forwards secondary to ColorReplace; Intercom item ($50 — shows customer plant request from anywhere) added to buy_scene; Water Drone ($10 — background autonomous watering, 3D adaptation: pure logic no sprite) added to buy_scene + game_state.has_drone; `sound.lua` replaced with multi-track system (menu + bg with crossfade, `play_animalese`) — `Sound.update` wired in main.lua; animalese voice sounds play per typewriter character using per-character `voice_pitch`; start screen fades to menu music on enter; settings menu "Leave Game" → "Main Menu" label when opened in-game; window title updated; three new test files (`test_water_drone.lua`, `test_intercom.lua`, `test_customer_scripts.lua`)

- **wip-parity-4** — Four features ported from wip: start-screen sub-logo (`assets/sub_logo.png`) + tagline drawn below the main logo with `BTN_Y0` shifted to 360; `_hud_labels()` now shows F: SKIP / F: CONTINUE during `talking_after` state; fade-to-black scene transition (0.3 s out+in) in `scene_manager.lua` with `LOGICAL_W`/`LOGICAL_H` added to `config.lua`; Marketing (customer cooldown) upgrade — `cooldown_tiers.lua`, `cooldown_level` in `game_state.lua`, `A.ads` in `assets.lua`, Marketing entry (#12) in `buy_scene.lua`, `spawn_cooldown()` + cd=0 fast-path in `store_scene.lua`; three new test files (`test_scene_manager.lua`, `test_start_scene.lua`, `test_shop.lua`)
