...
- **Passage fixed at north** — separator opening moved from the two center slot rows to the two northernmost slot rows (rows 1 and 2), so the store always expands southward without shifting the cashier entrance; tests updated to navigate through the new passage position
